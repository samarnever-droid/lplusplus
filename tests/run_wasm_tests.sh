...
fn parse_object(bytes: &[u8], path: &Path) -> Result<ObjectImage, String> {
    ...
    for sym in file.symbols() {
        ...
        if matches!(
            sym.kind(),
            SymbolKind::File | SymbolKind::Section | SymbolKind::Null
        ) {
            continue;
        }
...
pub fn write_elf_with_options(
    ...
    let text_file = align_up(ro_cursor, page);
    ...
    let dynsym_count = if dyn_mode { 1 + plt_keys.len() } else { 0 };
    ...
    let data_va = align_up_u64(text_va + rx_size as u64, page as u64);
    // Choose data_file such that data_file % page == data_va % page.
    let want_mod = congruent_offset(data_va, page as u64) as usize;
    let mut data_file = align_up(text_file + rx_filesz, page);
    // bump until congruence holds (usually already true if both page-aligned)
    while data_file % page != want_mod {
        data_file += 1;
        if data_file > text_file + rx_filesz + page {
            data_file = align_up(text_file + rx_filesz, page) + want_mod;
            break;
        }
    }

    let dynamic_ents = if dyn_mode {
        // NEEDED* + HASH STRTAB SYMTAB STRSZ SYMENT PLTGOT PLTRELSZ PLTREL JMPREL DEBUG FLAGS FLAGS_1 NULL
        dyn_needed.len() + 14
    } else {
        0
    };
        ...
        match machine {
            ...
            Machine::Aarch64 => {
                ...
                for (i, (_name, _)) in plt_ordered.iter().enumerate() {
                    ...
                    let delta = page(slot) as i64 - page(plt_va_i) as i64;
                    let imm = delta >> 12;
                    let immlo = (imm as u32) & 3;
                    let immhi = ((imm as u32) >> 2) & 0x7_ffff;
                    let adrp = 0x90000010u32 | (immlo << 29) | (immhi << 5);
...
fn build_imports(
    ...
    for imp in raw_imports {
        ...
        let dll = classify_dll(&clean, extra).unwrap_or_else(|| "KERNEL32.dll".into());
...
pub fn write_pe_with_options(
    ...
    // Auto: treat leftovers as KERNEL32 imports only if they look like Win32
    for u in leftover {
...
pub fn write_macho_with_options(
    ...
    let text_fileoff = align_up(header_and_cmds, page);
    let text_size = merged.text.len();
    let text_seg_filesz = align_up(text_fileoff + text_size, page);
    ...
    let data_seg_filesz = align_up(data_size.max(1), page);
    ...
    for sy in &names {
        ...
        rebuilt.push(macho_nlist(strx, 0x0f, n_sect, 0, va));
    }
    for n in &import_ord {
        let needle = {
            let mut s = Vec::new();
        ...
        let strx = strtab
            ...
            .unwrap_or(0) as u32;
        rebuilt.push(macho_nlist(strx, 0x01, 0, 0x0100, 0));
    }
    let nlist_bytes: Vec<u8> = rebuilt.iter().flatten().copied().collect();
    ...
    for (name, &i) in &stub_index {
        ...
        match machine {
            ...
            Machine::Aarch64 => {
                let delta = page(got_va) as i64 - page(stub_va) as i64;
                let imm = delta >> 12;
                let immlo = (imm as u32) & 3;
                let immhi = ((imm as u32) >> 2) & 0x7_ffff;
                let adrp = 0x90000010u32 | (immlo << 29) | (immhi << 5);
    ...
    let sig = macho_adhoc_codesig(&bin[..code_limit], &ident, page, text_seg_vmsize);
    ...
    let le_vmsize = align_up(le_total, page) as u64;
...
mod tests {
    ...
    #[test]
    fn elf_hash_foo() {
        // Known SysV hash of "foo"
        assert_eq!(elf_hash(b"foo"), 0x6f6f66 >> 0 | {
            // just check stability / non-zero
            1
        } * elf_hash(b"foo"));
        assert_ne!(elf_hash(b"foo"), 0);
