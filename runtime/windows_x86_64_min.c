...
typedef struct LppMapEntry { int64_t key; int64_t val; int occupied; } LppMapEntry;
typedef struct LppMap { LppMapEntry *entries; int64_t cap; int64_t len; } LppMap;
static uint64_t lpp_map_hash_key(int64_t key) { if (key >= 0x400000) { const char *s = (const char *)(uintptr_t)key; uint64_t hash = 14695981039346656037ULL; size_t len = 0; int is_str = 1; while (s[len] && len < 4096) { unsigned char c = (unsigned char)s[len]; if (c < 32 && c != '\n' && c != '\r' && c != '\t') { is_str = 0; break; } hash ^= c; hash *= 1099511628211ULL; len++; } if (is_str && len > 0) return hash; } uint64_t k = (uint64_t)key; k = (~k) + (k << 21); k = k ^ (k >> 24); k = (k + (k << 3)) + (k << 8); k = k ^ (k >> 14); k = (k + (k << 2)) + (k << 4); k = k ^ (k >> 28); k = k + (k << 31); return k; }
static int lpp_map_key_equal(int64_t k1, int64_t k2) { if (k1 == k2) return 1; if (k1 >= 0x400000 && k2 >= 0x400000) { const char *s1 = (const char *)(uintptr_t)k1; const char *s2 = (const char *)(uintptr_t)k2; if (lpp_strcmp(s1, s2) == 0) return 1; } return 0; }
...
static void lpp_map_rehash(LppMap *m) { int64_t old_cap = m->cap; LppMapEntry *old_entries = m->entries; m->cap = old_cap * 2; m->entries = (LppMapEntry *)VirtualAlloc(0, lpp_page_round((uint64_t)m->cap * sizeof(LppMapEntry)), MEM_COMMIT|MEM_RESERVE, PAGE_READWRITE); m->len = 0; for (int64_t i = 0; i < old_cap; i++) { if (old_entries[i].occupied == 1) { int64_t key = old_entries[i].key; int64_t val = old_entries[i].val; uint64_t h = lpp_map_hash_key(key); int64_t idx = (int64_t)(h % (uint64_t)m->cap); while (m->entries[idx].occupied == 1) { idx = (idx + 1) % m->cap; } m->entries[idx].key = key; m->entries[idx].val = val; m->entries[idx].occupied = 1; m->len++; } } if (old_entries) VirtualFree(old_entries, 0, MEM_RELEASE); }
void lpp_map_put(void *map, int64_t key, int64_t val) { LppMap *m = (LppMap *)map; if (!m) return; if (m->len * 10 >= m->cap * 7) { lpp_map_rehash(m); } uint64_t h = lpp_map_hash_key(key); int64_t idx = (int64_t)(h % (uint64_t)m->cap); int64_t first_tombstone = -1; while (m->entries[idx].occupied != 0) { if (m->entries[idx].occupied == 1 && lpp_map_key_equal(m->entries[idx].key, key)) { m->entries[idx].val = val; return; } if (m->entries[idx].occupied == 2 && first_tombstone == -1) { first_tombstone = idx; } idx = (idx + 1) % m->cap; } if (first_tombstone != -1) { idx = first_tombstone; } m->entries[idx].key = key; m->entries[idx].val = val; m->entries[idx].occupied = 1; m->len++; }
int64_t lpp_map_get(void *map, int64_t key) { LppMap *m = (LppMap *)map; if (!m || m->len == 0) return 0; uint64_t h = lpp_map_hash_key(key); int64_t idx = (int64_t)(h % (uint64_t)m->cap); int64_t start_idx = idx; while (m->entries[idx].occupied != 0) { if (m->entries[idx].occupied == 1 && lpp_map_key_equal(m->entries[idx].key, key)) { return m->entries[idx].val; } idx = (idx + 1) % m->cap; if (idx == start_idx) break; } return 0; }
int64_t lpp_map_has(void *map, int64_t key) { LppMap *m = (LppMap *)map; if (!m || m->len == 0) return 0; uint64_t h = lpp_map_hash_key(key); int64_t idx = (int64_t)(h % (uint64_t)m->cap); int64_t start_idx = idx; while (m->entries[idx].occupied != 0) { if (m->entries[idx].occupied == 1 && lpp_map_key_equal(m->entries[idx].key, key)) { return 1; } idx = (idx + 1) % m->cap; if (idx == start_idx) break; } return 0; }
int64_t lpp_map_len(void *map) { LppMap *m = (LppMap *)map; return m ? m->len : 0; }
void lpp_map_remove(void *map, int64_t key) { LppMap *m = (LppMap *)map; if (!m || m->len == 0) return; uint64_t h = lpp_map_hash_key(key); int64_t idx = (int64_t)(h % (uint64_t)m->cap); int64_t start_idx = idx; while (m->entries[idx].occupied != 0) { if (m->entries[idx].occupied == 1 && lpp_map_key_equal(m->entries[idx].key, key)) { m->entries[idx].occupied = 2; m->len--; return; } idx = (idx + 1) % m->cap; if (idx == start_idx) break; } }
