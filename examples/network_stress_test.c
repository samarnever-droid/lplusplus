...
typedef struct __lpp_closure_env_7 __lpp_closure_env_7_t;
struct __lpp_closure_env_7 {
    int64_t cap_0;
};

static void lpp_drop___lpp_closure_env_7(void* raw) {
    __lpp_closure_env_7_t* self = (__lpp_closure_env_7_t*)raw;
}

int64_t handle_worker(int64_t client);
void lpp_main(void);
...
static void lpp__spawn_fn_0(lpp__spawn_env_0_t* __env) {
    int64_t client_6 = __env->client_6;
    return handle_worker(client_6);
}
...
int64_t handle_worker(int64_t client_2) {
    if ((client_2 >= 0)) {
        /* Storage: Value */
        char* msg_3 = lpp_net_recv(client_2, 512);
        if ((lpp_list_len(msg_3) > 0)) {
            lpp_net_send_all(client_2, "OK:");
            lpp_net_send_all(client_2, msg_3);
        }
        lpp_net_close(client_2);
    }
    return 0;
}

void lpp_main(void) {
    lpp_print_str("[L++] Starting High-Concurrency TCP Stress Server on port 9090...");
    /* Storage: Value */
    int64_t listener_4 = lpp_net_listen(9090);
    lpp_print_str("[L++] Server listening on port 9090 (fd:");
    printf("%lld\n", (long long)(listener_4));
    /* Storage: Value */
    int64_t count_5 = 0;
    while ((count_5 < 50)) {
        /* Storage: Value */
        int64_t client_6 = lpp_net_accept(listener_4);
        if ((client_6 >= 0)) {
            count_5 = (count_5 + 1);
            lpp__spawn_env_0_t* __lpp_senv_0 = (lpp__spawn_env_0_t*)malloc(sizeof(lpp__spawn_env_0_t));
            __lpp_senv_0->client_6  = client_6;
            lpp_thread_spawn(lpp__spawn_wrapper_0, __lpp_senv_0);
        }
    }
    lpp_print_str("[L++] Processed 50 connections successfully!");
    lpp_net_close(listener_4);
}
