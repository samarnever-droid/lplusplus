...
typedef struct __lpp_closure_env_4 __lpp_closure_env_4_t;
struct __lpp_closure_env_4 {
    int64_t cap_0;
};

static void lpp_drop___lpp_closure_env_4(void* raw) {
    __lpp_closure_env_4_t* self = (__lpp_closure_env_4_t*)raw;
}

int64_t handle_client(int64_t listener);
void lpp_main(void);
...
static void lpp__spawn_fn_0(lpp__spawn_env_0_t* __env) {
    int64_t listener_7 = __env->listener_7;
    return handle_client(listener_7);
}

static void lpp__spawn_wrapper_0(void* __raw) {
    lpp__spawn_env_0_t* __env = (lpp__spawn_env_0_t*)__raw;
    lpp__spawn_fn_0(__env);
    free(__raw);
}


int64_t handle_client(int64_t listener_2) {
    lpp_print_str("accepting...");
    /* Storage: Value */
    int64_t client_3 = lpp_net_accept(listener_2);
    lpp_print_str("client accepted:");
    printf("%lld\n", (long long)(client_3));
    if ((client_3 >= 0)) {
        /* Storage: Value */
        char* msg_4 = lpp_net_recv(client_3, 1024);
        lpp_print_str("received:");
        lpp_print_str(msg_4);
        /* Storage: Value */
        int64_t sent_5 = lpp_net_send_all(client_3, msg_4);
        lpp_print_str("echoed bytes:");
        printf("%lld\n", (long long)(sent_5));
        lpp_net_close(client_3);
    }
    lpp_net_close(listener_2);
    return 0;
}

void lpp_main(void) {
    /* Storage: Value */
    char* ip_6 = lpp_net_resolve("localhost");
    lpp_print_str("resolve localhost:");
    lpp_print_str(ip_6);
    lpp_print_str("starting echo server on 8081...");
    /* Storage: Value */
    int64_t listener_7 = lpp_net_listen(8081);
    lpp_print_str("listener fd:");
    printf("%lld\n", (long long)(listener_7));
    lpp__spawn_env_0_t* __lpp_senv_0 = (lpp__spawn_env_0_t*)malloc(sizeof(lpp__spawn_env_0_t));
    __lpp_senv_0->listener_7  = listener_7;
    lpp_thread_spawn(lpp__spawn_wrapper_0, __lpp_senv_0);
}
