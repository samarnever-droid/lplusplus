...
fn main() {
    ...
    while idx < args.len() {
        ...
        if arg == "--version" || arg == "-v" {
            println!("L++ Compiler v4.2.1 (Pure Native AOT)");
            return;
        } else if arg == "--help" || arg == "-h" {
            println!("L++ (L Plus Plus) v4.2.1 — Pure Native Compiler & Toolchain");
            ...
            println!();
            println!("Language Features (v4.2.1):");
    ...
    match escape::EscapeAnalyzer::analyze(&ast, &resolver.table, &type_table) {
        Ok(storage) => {
            ...
            if env::var("BENCHMARK").is_ok() {
                ...
                && !dump_mir
            {
                println!("L++ v4.2.1 (Pure Native Executable)\n");
