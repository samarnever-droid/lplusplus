...
fn main() {
    ...
    for arg in args.iter().skip(1) {
        if arg == "--version" || arg == "-v" {
            println!("L++ Compiler v3.2.0 (Pure Native AOT)");
            return;
        } else if arg == "--help" || arg == "-h" {
            println!("L++ (L Plus Plus) v3.2.0 — Pure Native Compiler & Toolchain");
            ...
            println!();
            println!("Language Features (v3.2.0):");
    ...
    match escape::EscapeAnalyzer::analyze(&ast, &resolver.table, &type_table) {
        Ok(storage) => {
            ...
            if env::var("BENCHMARK").is_ok() {
                ...
                && !dump_mir
            {
                println!("L++ v3.2.0 (Pure Native Executable)\n");
