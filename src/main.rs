...
fn real_main() -> i32 {
    ...
    if args.len() > 1 && args[1] == "emit" {
        // `emit` is always a source command. It requires a file, but we strip it from args
        // so it doesn't get treated as the filename if there's no file.
        is_emit_cmd = true;
        args.remove(1);
    } else if args.len() > 1 && args[1] == "check" {
        // Disambiguate `lpp check` (PM) from `lpp check file.lpp` (Source)
        let is_source = args
            .iter()
            .skip(2)
            .any(|arg| arg.ends_with(".lpp") || (!arg.starts_with('-') && Path::new(arg).exists()));
        if is_source {
            source_check_command = true;
            args.remove(1);
        }
    } else if args.len() > 1 && args[1] == "run" {
        // Disambiguate `lpp run` (PM) from `lpp run file.lpp` (Source)
        let is_source = args
            .iter()
            .skip(2)
            .any(|arg| arg.ends_with(".lpp") || (!arg.starts_with('-') && Path::new(arg).exists()));
    ...
    if args.len() > 1 {
        ...
        if first_arg == "init"
            ...
            || first_arg == "login"
            || first_arg == "upgrade"
            || first_arg == "self-update"
            || first_arg == "update-self"
            || first_arg == "workspace"
            || first_arg == "help"
            || first_arg == "bench"
        {
