                    ...
                    match rvalue {
                        ...
                        Rvalue::BinaryOp(op, left, right) => {
                            let (l_known, r_known) = match (&*left, &*right) {
                                (Operand::Local(l), Operand::Local(r)) => {
                                    (known.get(l), known.get(r))
                                }
...
mod tests {
    ...
    fn folds_add_of_constants() {
        ...
        assert!(matches!(
            last,
            MirInstr::Assign(LocalId(2), Rvalue::Use(Operand::Int(7)))
        ));
    }
    ...
    fn propagates_through_chain() {
        ...
        assert!(matches!(
            last,
            MirInstr::Assign(LocalId(3), Rvalue::Use(Operand::Int(15)))
        ));
    }
}
