...
fn add_operand_use(live: &mut HashSet<LocalId>, operand: &Operand) {
    if let Operand::Local(id) | Operand::Borrowed(id) = operand { live.insert(*id); }
}

fn pure_nontrapping(rvalue: &Rvalue) -> bool {
    matches!(rvalue,
        Rvalue::Use(_) |
        Rvalue::BinaryOp(BinaryOperator::Add | BinaryOperator::Subtract | BinaryOperator::Multiply |
                          BinaryOperator::Eq | BinaryOperator::NotEq | BinaryOperator::Less |
                          BinaryOperator::Greater | BinaryOperator::LessEq | BinaryOperator::GreaterEq, _, _)
    )
}
...
fn add_rvalue_uses(live: &mut HashSet<LocalId>, rvalue: &Rvalue) {
    match rvalue {
        ...
        Rvalue::BinaryOp(_, left, right) => { add_operand_use(live, left); add_operand_use(live, right); }
        ...
        Rvalue::CallDirect(_, args) | Rvalue::CallIndirect(_, args) | Rvalue::BuiltinCall(_, args) | Rvalue::MakeClosure(_, args) => {
            for value in args { add_operand_use(live, value); }
        }
...
pub fn run(program: &mut MirProgram) -> usize {
    ...
    for function in program.functions.values_mut() {
        if function.blocks.len() != 1 { continue; }
        ...
        for instruction in block.instrs.drain(..).rev() {
            match &instruction {
                ...
                MirInstr::AssignField { base, value, .. } => {
                    live.insert(*base); add_operand_use(&mut live, value); kept.push(instruction);
                }
                MirInstr::Retain(local) | MirInstr::Release(local) => { live.insert(*local); kept.push(instruction); }
            }
        }
        kept.reverse(); block.instrs = kept;
    }
    removed
}
