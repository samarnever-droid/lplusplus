...
fn remap_operand(op: &Operand, map: &HashMap<LocalId, Operand>) -> Option<Operand> {
    match op {
        ...
        Operand::Int(_) | Operand::Bool(_) => Some(op.clone()),
        _ => None,
    }
}
...
fn candidate(function: &MirFunction) -> Option<Candidate> {
    ...
    if !matches!(
        result,
        Operand::Local(_) | Operand::Int(_) | Operand::Bool(_)
    ) {
        return None;
    }
    for instruction in &block.instrs {
        match instruction {
            MirInstr::Assign(
                _,
                Rvalue::Use(Operand::Local(_) | Operand::Int(_) | Operand::Bool(_)),
            ) => {}
    ...
    if function
        ...
        .iter()
        .skip(1)
        .any(|block| !block.instrs.is_empty())
    {
        return None;
    }
    Some(Candidate {
...
pub fn run(program: &mut MirProgram) -> usize {
    let candidates: HashMap<FuncId, Candidate> = program
        .functions
        .iter()
        .filter_map(|(id, function)| candidate(function).map(|candidate| (*id, candidate)))
        .collect();
    let mut inlined = 0;
    for function in program.functions.values_mut() {
        for block in &mut function.blocks {
            ...
            for instruction in original {
                let MirInstr::Assign(destination, Rvalue::CallDirect(target, args)) = &instruction
                else {
                ...
                if candidate.params.len() != args.len() || *target == function.id {
                    expanded.push(instruction);
                    continue;
                }
                let mut map: HashMap<LocalId, Operand> = candidate
                    .params
                    .iter()
                    .copied()
                    .zip(args.iter().cloned())
                    .collect();
                let returned = match &candidate.result {
                ...
                for candidate_instruction in &candidate.instrs {
                    let MirInstr::Assign(old_dest, rvalue) = candidate_instruction else {
                        valid = false;
                        break;
                    };
                    let new_dest = if Some(*old_dest) == returned {
                        *destination
                    } else {
                        ...
                        local.id = id;
                        local.debug_name = Some(format!("inline_{}_{}", target.0, old_dest.0));
                        function.locals.push(local);
                        map.insert(*old_dest, Operand::Local(id));
                        id
                    };
                    let new_rvalue = match rvalue {
                        Rvalue::Use(value) => remap_operand(value, &map).map(Rvalue::Use),
                        Rvalue::BinaryOp(op, left, right) => {
                            match (remap_operand(left, &map), remap_operand(right, &map)) {
                                (Some(left), Some(right)) => {
                                    Some(Rvalue::BinaryOp(op.clone(), left, right))
                                }
                                _ => None,
                            }
                        }
                    ...
                    let Some(new_rvalue) = new_rvalue else {
                        valid = false;
                        break;
                    };
                ...
                if valid {
                    if !matches!(candidate.result, Operand::Local(_)) {
                        if let Some(value) = remap_operand(&candidate.result, &map) {
                            generated.push(MirInstr::Assign(*destination, Rvalue::Use(value)));
                        }
                    }
