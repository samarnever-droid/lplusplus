...
fn cmd_check(args: &[String]) -> i32 {
    let is_wasm = args
        .iter()
        .any(|a| a == "--wasm" || a == "-w" || a == "--backend=wasm")
        || args.windows(2).any(|w| {
            w[0] == "--target" && (w[1].starts_with("wasm32") || w[1].starts_with("wasm64"))
        });
...
fn cmd_check_wasm(_args: &[String]) -> i32 {
    ...
    for file_path in &scanned_files {
        ...
        for (line_no, line) in content.lines().enumerate() {
            ...
            if l.contains("net_listen")
                || l.contains("net_accept")
                || l.contains("net_read")
                || l.contains("net_write")
                || l.contains("net_dial")
            {
                net_issues.push(format!(
                    "  • {}:{} -> Network Socket API (WASI preview1 has no raw TCP sockets)",
                    file_str,
                    line_no + 1
                ));
                has_issue = true;
            }
            if l.contains("read_file")
                || l.contains("write_file")
                || l.contains("append_file")
                || l.contains("file_exists")
                || l.contains("delete_file")
            {
                file_issues.push(format!(
                    "  • {}:{} -> File I/O API (Requires WASI file descriptor preopen shims)",
                    file_str,
                    line_no + 1
                ));
                has_issue = true;
            ...
            if l.contains("VectorI64x2") || l.contains("simd_") {
                simd_issues.push(format!(
                    "  • {}:{} -> Native SIMD Types (Wasm backend uses scalar/SWAR algorithms)",
                    file_str,
                    line_no + 1
                ));
                has_issue = true;
    ...
    println!(
        "  • Fully WASM-Compatible Files: {} (100% ready for WebAssembly)",
        fully_compatible.len()
    );
    let issues_count = net_issues.len()
        + file_issues.len()
        + thread_issues.len()
        + ffi_issues.len()
        + simd_issues.len();
    ...
    if !net_issues.is_empty() {
        println!(
            "  \x1b[1;33m[!] Network Socket Plane ({} occurrences):\x1b[0m",
            net_issues.len()
        );
        for item in &net_issues {
            println!("{}", item);
        }
        println!(
            "      \x1b[1;36m-> Fix for Wasm:\x1b[0m In WebAssembly / Cloudflare Workers, replace TCP listen with HTTP POST /query.\n"
        );
    }
    if !file_issues.is_empty() {
        println!(
            "  \x1b[1;33m[!] Host File System Plane ({} occurrences):\x1b[0m",
            file_issues.len()
        );
        ...
        println!(
            "      \x1b[1;36m-> Fix for Wasm:\x1b[0m In WebAssembly, use In-Memory Virtual Pages or Cloudflare R2 / S3 Object Storage.\n"
        );
    }
    if !thread_issues.is_empty() {
        println!(
            "  \x1b[1;33m[!] OS Thread Plane ({} occurrences):\x1b[0m",
            thread_issues.len()
        );
        for item in &thread_issues {
            println!("{}", item);
        }
        println!(
            "      \x1b[1;36m-> Fix for Wasm:\x1b[0m Use L++ single-thread async/await task executors.\n"
        );
    }
    if !ffi_issues.is_empty() {
        println!(
            "  \x1b[1;33m[!] Host C FFI Symbols ({} occurrences):\x1b[0m",
            ffi_issues.len()
        );
        for item in &ffi_issues {
            println!("{}", item);
        }
    }
    if !simd_issues.is_empty() {
        println!(
            "  \x1b[1;33m[!] Native SIMD Types ({} occurrences):\x1b[0m",
            simd_issues.len()
        );
        for item in &simd_issues {
            println!("{}", item);
        }
    }
    println!();
