...
/// displacement field.
fn write_macho_arm64(_inputs: &[PathBuf], _output: &Path) -> Result<(), String> {
    // Production macOS forbids static MH_EXECUTE binaries on arm64. Emitting a
    // runtime-free static image produces a process that is killed by the OS
    // policy loader rather than a useful executable. ARM64 must wait for the
    // dynamic libSystem/import-table path (M3), so reject it explicitly.
    Err("Direct static Mach-O ARM64 is unsupported by macOS policy; dynamic libSystem imports are required before lpp-link macho-arm64 can emit executables.".to_string())
}
