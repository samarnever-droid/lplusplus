...
//! `macho` subcommands still work for cross-compilation and backward compat.
//! ELF with GOT/rodata merge, PE with multi-section layout (.text/.rdata/.data/
//! .idata/.reloc), base relocations, and broad AMD64 relocation coverage.
//! Mach-O direct emitter for the verified subset.
//!
...
/// The output format, either explicitly requested or auto-detected from host OS.
...
impl OutputFormat {
    /// Auto-detect: picks the right format for the host OS.
    fn for_host() -> Self {
...
/// What kind of section a relocation lives in.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum SectionClass {
    Text,
    Rodata,
...
fn read_elf_input(path: &Path) -> Result<ElfInput, String> {
    ...
    for sym in file.symbols() {
        ...
        if let Some(dst) = dst {
            if let Ok(n) = sym.name() {
                if !n.is_empty() { dst.push((n.to_string(), sym.address())); }
            }
        }
    }
    ...
    Ok(ElfInput {
        path: path.to_path_buf(), text, rodata,
        text_symbols: text_syms, rodata_symbols: rodata_syms, relocations: relocs,
    })
}
...
fn write_elf(inputs: &[PathBuf], output: &Path) -> Result<(), String> {
    ...
    // A runtime-free ELF may have `main` but no `lpp_main`.  Treat either as
    // the entry symbol; prefer `main` (the C ABI wrapper) when present.
    let entry = syms.get("main").or_else(|| syms.get("lpp_main"))
        .ok_or_else(|| "required symbol 'main' (or 'lpp_main') not found".to_string())?;
    // Keep the startup stub simple: we call whatever entry we found.
    ...
    if disp < i32::MIN as i64 || disp > i32::MAX as i64 {
        return Err("entry is out of range for startup call".to_string());
    }
    let mut start = vec![
    ...
    for (idx, inp) in objs.iter().enumerate() {
        ...
        for rel in &inp.relocations {
            if rel.size != 32 {
                return Err(format!("'{}': unsupported relocation width {}", inp.path.display(), rel.size));
            }
            let tgt = match rel.kind {
                ...
                _ => *syms.get(&rel.target).ok_or_else(|| {
                    format!("'{}': unresolved external '{}'", inp.path.display(), rel.target)
                })?,
            };
            let patch = base + rel.offset;
            if patch + 4 > text.len() { return Err(format!("'{}': patch out of range", inp.path.display())); }
    ...
    put_u16(&mut elf, 16, 2);  put_u16(&mut elf, 18, EM_X86_64);
...
fn read_coff_full(path: &Path) -> Result<CoffSections, String> {
    ...
    for sec in file.sections() {
        ...
        for (off, rel) in sec.relocations() {
            ...
            let sym = file.symbol_by_index(si)
                .map_err(|e| format!("read relocation symbol: {e}"))?;
            let raw_name = sym.name()
                .map_err(|e| format!("read relocation symbol name: {e}"))?;
            let target = resolve_coff_target(&raw_name, &sym, &map, class);
            relocs.push(Relocation {
                offset: base + raw_off, target,
                addend: rel.addend(), size: rel.size(), kind: rel.kind(),
            });
        }
            ...
            if let Some((_, class, base)) = map.iter().find(|(i, _, _)| *i == idx) {
                if let Ok(name) = sym.name() {
                    if !name.is_empty() && !name.starts_with(".text")
                        ...
                        && !name.starts_with(".bss") && !name.starts_with('$')
                    {
...
fn resolve_coff_target(
    raw_name: &str, sym: &object::Symbol<'_, '_>,
    map: &[(object::SectionIndex, SectionClass, usize)], self_class: SectionClass,
) -> String {
    let is_anonymous = raw_name.is_empty() || sym.kind() == object::SymbolKind::Section
...
struct ImportData {
    ...
    #[allow(dead_code)] ilt_rva: u32, iat_rva: u32,
    #[allow(dead_code)] dll_count: usize,
}
...
fn build_imports(kernel_imports: &[String], refptrs: &[String], section_rva: u32) -> Result<ImportData, String> {
    ...
    if count > 0 {
        ...
        put_u32(&mut data, 0, section_rva + ilt_off as u32);
        put_u32(&mut data, 12, section_rva + dll_off as u32);
...
fn write_pe(inputs: &[PathBuf], output: &Path) -> Result<(), String> {
    ...
    for obj in &objs {
        for rel in &obj.relocations {
            if let Some(name) = rel.target.strip_prefix("__imp_") {
                ...
                if !kernel_imports.contains(&n) { kernel_imports.push(n); }
            } else if let Some(name) = rel.target.strip_prefix(".refptr.") {
                let n = name.to_string();
                if !refptr_names.contains(&n) { refptr_names.push(n); }
            }
        }
    }
    ...
    if has_idata { all_writable.extend_from_slice(&import.data); }
    let reloc_data = generate_base_relocs(&all_writable, data_rva);
    let reloc_rva = if !reloc_data.is_empty() {
        pe_align(if has_idata { idata_rva as usize + import.data.len() } else { data_rva as usize + merged_data.len() }, PE_SECT_ALIGN) as u32
    } else { 0 };
    let has_reloc = !reloc_data.is_empty();
    let reloc_raw_size = if has_reloc { pe_align(reloc_data.len(), PE_FILE_ALIGN) } else { 0 };

    for (idx, obj) in objs.iter().enumerate() {
        let b = &bases[idx];
        for rel in &obj.relocations {
            let patch_class = section_class_for_offset(
                rel.offset, b.text_base, merged_text.len(),
                b.rdata_base, merged_rdata.len(), b.data_base, merged_data.len());
            ...
            match rnum {
                AMD64_ADDR64 => {
                    if patch + 8 > patch_buf.len() { return Err(format!("'{}': ADDR64 patch out of range", obj.path.display())); }
                    patch_buf[patch..patch + 8].copy_from_slice(&(target as u64).to_le_bytes());
                }
                AMD64_ADDR32 | AMD64_ADDR32NB => {
                    if patch + 4 > patch_buf.len() { return Err(format!("'{}': ADDR32 patch out of range", obj.path.display())); }
                    let val = target;
                    if val < i32::MIN as u64 || val > i32::MAX as u64 { return Err(format!("'{}': ADDR32 value {val:#x} out of range", obj.path.display())); }
                    patch_buf[patch..patch + 4].copy_from_slice(&(val as i32).to_le_bytes());
                }
                AMD64_REL32 | AMD64_REL32_1 | AMD64_REL32_2 | AMD64_REL32_3 | AMD64_REL32_4 | AMD64_REL32_5 => {
                    if patch + 4 > patch_buf.len() { return Err(format!("'{}': REL32 patch out of range", obj.path.display())); }
                    let adjustment: i64 = match rnum {
                        AMD64_REL32_1 => 1, AMD64_REL32_2 => 2, AMD64_REL32_3 => 3,
                        AMD64_REL32_4 => 4, AMD64_REL32_5 => 5, _ => 0,
                    };
                    let disp = target as i64 + rel.addend - (patch_rva_addr + 4 + adjustment);
                    if disp < i32::MIN as i64 || disp > i32::MAX as i64 { return Err(format!("'{}': REL32 displacement out of range ({disp})", obj.path.display())); }
                    patch_buf[patch..patch + 4].copy_from_slice(&(disp as i32).to_le_bytes());
                }
                AMD64_SECTION => { /* skip */ }
                AMD64_SECREL => {
                    if patch + 4 > patch_buf.len() { return Err(format!("'{}': SECREL patch out of range", obj.path.display())); }
                    if target > u32::MAX as u64 { return Err(format!("'{}': SECREL value overflow", obj.path.display())); }
                    patch_buf[patch..patch + 4].copy_from_slice(&(target as u32).to_le_bytes());
                }
                _ => return Err(format!("'{}': unsupported COFF relocation type {rnum}", obj.path.display())),
            }
        }
    }
    ...
    for (i, name) in refptr_names.iter().enumerate() {
        ...
        if pos + 8 > merged_data.len() { continue; }
    ...
    let main_abs = global_syms.get("main")
        .map(|(c, a)| match c { SectionClass::Text => text_rva as u64 + a, _ => text_rva as u64 + a })
    ...
    let dirs = opt + 112;
    if has_idata {
        put_u32(&mut pe, dirs + 8, idata_rva); put_u32(&mut pe, dirs + 12, import.data.len() as u32);
        put_u32(&mut pe, dirs + 12 * 8, import.iat_rva);
        put_u32(&mut pe, dirs + 12 * 8 + 4, ((kernel_imports.len() + 1) * 8) as u32);
    }
    if has_reloc {
        put_u32(&mut pe, dirs + 5 * 8, reloc_rva); put_u32(&mut pe, dirs + 5 * 8 + 4, reloc_data.len() as u32);
    }

    let mut sec = opt + opt_size as usize;
    let emit_section = |pe: &mut [u8], sec: &mut usize, name: &[u8; 8], rva: u32, raw_size: usize, raw_off: usize, virt_size: usize, characteristics: u32| {
        pe[*sec..*sec + 8].copy_from_slice(name);
        put_u32(pe, *sec + 8, virt_size as u32); put_u32(pe, *sec + 12, rva);
        put_u32(pe, *sec + 16, raw_size as u32); put_u32(pe, *sec + 20, raw_off as u32);
        put_u32(pe, *sec + 36, characteristics);
        *sec += 40;
    };

    let mut tname = [0u8; 8]; tname[..5].copy_from_slice(b".text");
    emit_section(&mut pe, &mut sec, &tname, text_rva, text_raw_size, text_raw_off, merged_text.len(), 0x60000020);
    if !merged_rdata.is_empty() {
        let mut rname = [0u8; 8]; rname[..6].copy_from_slice(b".rdata");
        emit_section(&mut pe, &mut sec, &rname, rdata_rva, rdata_raw_size, rdata_raw_off, merged_rdata.len(), 0x40000040);
    }
    if !merged_data.is_empty() || !refptr_names.is_empty() {
        let mut dname = [0u8; 8]; dname[..5].copy_from_slice(b".data");
        emit_section(&mut pe, &mut sec, &dname, data_rva, data_raw_size, data_raw_off, merged_data.len(), 0xC0000040);
    }
    if has_idata {
        let mut iname = [0u8; 8]; iname[..6].copy_from_slice(b".idata");
        emit_section(&mut pe, &mut sec, &iname, idata_rva, idata_raw_size, idata_raw_off, import.data.len(), 0xC0000040);
    }
    if has_reloc {
        let mut rlname = [0u8; 8]; rlname[..6].copy_from_slice(b".reloc");
        emit_section(&mut pe, &mut sec, &rlname, reloc_rva, reloc_raw_size, reloc_raw_off, reloc_data.len(), 0x42000040);
    }
...
fn section_class_for_offset(
    offset: usize, text_base: usize, text_len: usize,
    rdata_base: usize, rdata_len: usize, data_base: usize, data_len: usize,
) -> SectionClass {
    ...
    else if offset >= data_base && offset < data_base + data_len { SectionClass::Data }
    else { SectionClass::Text }
}

fn resolve_pe_target(
    rel: &Relocation, global_syms: &HashMap<String, (SectionClass, u64)>,
    iat_rvas: &HashMap<String, u32>, _refptr_offsets: &HashMap<String, usize>,
    bases: &SectionBase, text_rva: u32, rdata_rva: u32, data_rva: u32, _idata_rva: u32,
) -> Result<u64, String> {
    ...
    if let Some(rest) = rel.target.strip_prefix("__ext_text__") {
        let eb: usize = rest.parse().map_err(|_| format!("invalid __ext_text__ tag: {}", rel.target))?;
        return Ok(PE_IMAGE_BASE + text_rva as u64 + eb as u64);
    }
    if let Some(rest) = rel.target.strip_prefix("__ext_rdata__") {
        let eb: usize = rest.parse().map_err(|_| format!("invalid __ext_rdata__ tag: {}", rel.target))?;
        return Ok(PE_IMAGE_BASE + rdata_rva as u64 + eb as u64);
    }
    if let Some(rest) = rel.target.strip_prefix("__ext_data__") {
        let eb: usize = rest.parse().map_err(|_| format!("invalid __ext_data__ tag: {}", rel.target))?;
        return Ok(PE_IMAGE_BASE + data_rva as u64 + eb as u64);
    }
    ...
    if let Some(rest) = rel.target.strip_prefix("__coff_text_section_") {
        let off: u32 = rest.parse().map_err(|_| "invalid __coff_text_section_ tag")?;
        return Ok(PE_IMAGE_BASE + text_rva as u64 + off as u64);
    }
    Err(format!("unresolved external COFF symbol '{}'", rel.target))
}

// ═══════════════════════════════════════════════════════════════════════════
//  3.  Mach-O path  (Phase M2 — kept stable)
// ═══════════════════════════════════════════════════════════════════════════
...
fn read_macho_input(path: &Path) -> Result<MachoInput, String> {
    ...
    if file.format() != BinaryFormat::MachO {
        return Err(format!("'{}' is not a Mach-O relocatable object", path.display()));
    }
    let mut text = Vec::new();
        ...
        if let SymbolSection::Section(idx) = sym.section() {
            if let Some(base) = find_base(idx) {
                if let Ok(name) = sym.name() {
                    ...
                    if !clean.is_empty() { text_syms.push((clean.to_string(), base as u64 + sym.address())); }
                }
            }
        }
    }
    ...
    for (_, base, off, rel) in sec_relocs {
        ...
        let target = if clean.is_empty() {
            match sym.section() {
                SymbolSection::Section(idx) if find_base(idx).is_some() => format!("__macho_text_section_{}", find_base(idx).unwrap()),
...
fn write_macho(inputs: &[PathBuf], output: &Path) -> Result<(), String> {
    ...
    let main = *syms.get("main").or_else(|| syms.get("_main"))
        .ok_or_else(|| "required symbol 'main' or '_main' not found".to_string())?;

    for (idx, inp) in objs.iter().enumerate() {
        let base = bases[idx];
        for rel in &inp.relocations {
            let tgt_off = if rel.target == "__self_text__" { base as u64 }
            else if let Some(off) = rel.target.strip_prefix("__macho_text_section_") { off.parse::<u64>().map_err(|_| "invalid Mach-O section relocation")? }
            else { *syms.get(&rel.target).ok_or_else(|| format!("'{}': unresolved external symbol '{}'", inp.path.display(), rel.target))? };
            let patch = base + rel.offset;
            if patch + 4 > text.len() { return Err(format!("'{}': patch out of range", inp.path.display())); }
    ...
    header.extend_from_slice(&3u32.to_le_bytes()); header.extend_from_slice(&2u32.to_le_bytes());
    header.extend_from_slice(&2u32.to_le_bytes());
    header.extend_from_slice(&((72 + 152) as u32).to_le_bytes());
    header.extend_from_slice(&0x00200085u32.to_le_bytes()); header.extend_from_slice(&0u32.to_le_bytes());
    // PAGEZERO
    ...
    header.extend_from_slice(&0u32.to_le_bytes()); header.extend_from_slice(&0u32.to_le_bytes());
    header.extend_from_slice(&0u32.to_le_bytes()); header.extend_from_slice(&0u32.to_le_bytes());
    // TEXT segment
    ...
    header.extend_from_slice(&0x100000000u64.to_le_bytes()); header.extend_from_slice(&(4096 + text_page as u64).to_le_bytes());
    header.extend_from_slice(&0u64.to_le_bytes()); header.extend_from_slice(&(4096 + text.len() as u64).to_le_bytes());
    header.extend_from_slice(&7u32.to_le_bytes()); header.extend_from_slice(&5u32.to_le_bytes());
    header.extend_from_slice(&1u32.to_le_bytes()); header.extend_from_slice(&0u32.to_le_bytes());
    // __text section
    ...
    fs::write(output, bin).map_err(|e| format!("write Mach-O binary '{}': {e}", output.display()))?;
    Ok(())
}

// ═══════════════════════════════════════════════════════════════════════════
//  4.  inspect  (cross-format)
// ═══════════════════════════════════════════════════════════════════════════
...
fn inspect_object(input: &Path) -> Result<(), String> {
    ...
    println!("sections:");
    for sec in file.sections() {
        for (_, rel) in sec.relocations() { reloc_count += 1; *reloc_kinds.entry(format!("{:?}", rel.kind())).or_default() += 1; }
        println!("  {} size={} kind={:?}", sec.name().unwrap_or("<unnamed>"), sec.size(), sec.kind());
    }
...
// ═══════════════════════════════════════════════════════════════════════════
//  5.  main  — auto-detects format from host OS
// ═══════════════════════════════════════════════════════════════════════════
...
fn usage() {
    ...
    eprintln!("The default mode auto-detects the format from the host OS:");
    eprintln!("  ELF on Linux, PE on Windows, Mach-O on macOS.");
}
...
fn main() {
    ...
    if args.first().map(String::as_str) == Some("--help") || args.first().map(String::as_str) == Some("-h") {
        usage(); return;
    }

    // Parse mode: explicit `pe` / `macho` override, or auto-detect from OS.
    let (mode, offset) = match args.first().map(String::as_str) {
        Some("pe") => (OutputFormat::PE, 1),
        Some("macho") => (OutputFormat::MachO, 1),
        _ => (OutputFormat::for_host(), 0),
    };
    ...
    let result = match mode {
        OutputFormat::PE => write_pe(&inputs, Path::new(&args[out_idx + 1])),
        OutputFormat::MachO => write_macho(&inputs, Path::new(&args[out_idx + 1])),
        OutputFormat::Elf => write_elf(&inputs, Path::new(&args[out_idx + 1])),
    };
