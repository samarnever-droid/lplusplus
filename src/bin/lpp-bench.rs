...
impl Platform {
    fn detect() -> Self {
        Self {
            ...
            exe_suffix: if cfg!(target_os = "windows") { ".exe" } else { "" },
            obj_suffix: if cfg!(target_os = "windows") { "obj" } else { "o" },
    ...
    fn runtime_flags(&self, cc: &str) -> Vec<String> {
        if cc.contains("cl.exe") || cc == "cl" {
            ...
            vec!["-O2".into(), "-ffreestanding".into(), "-fno-stack-protector".into(),
                 "-fno-pic".into(), "-mno-red-zone".into(), "-c".into()]
        }
    }
...
fn plat() -> Platform { Platform::detect() }
...
fn repo_root() -> PathBuf {
    ...
    loop {
        ...
        if let Some(parent) = dir.parent() { dir = parent.to_path_buf(); }
        else { return PathBuf::from("."); }
    }
}
...
fn disk_usage_kb(dir: &Path) -> u64 {
    if !dir.is_dir() { return file_size_bytes(dir) / 1024; }
    let mut total = 0u64;
    if let Ok(entries) = fs::read_dir(dir) {
        for e in entries.flatten() {
            let p = e.path();
            if p.is_dir() { total += disk_usage_kb(&p); }
            else { total += fs::metadata(&p).map(|m| m.len()).unwrap_or(0); }
        }
    }
    total / 1024
}
...
enum LinkerMode { Direct, Host, CTranspiler }
...
impl LinkerMode {
    ...
    fn short_label(&self) -> &'static str {
        match self { LinkerMode::Direct => "direct", LinkerMode::Host => "host", LinkerMode::CTranspiler => "c" }
    }
}
...
fn build_runtime_obj(tmp: &Path, freestanding_src: &Path) -> Result<PathBuf, String> {
    ...
    if status.success() { Ok(obj) } else { Err("runtime compile failed".into()) }
}

fn run_linker_direct(obj: &Path, runtime_obj: &Path, output: &Path) -> Result<LinkerResult, String> {
    let linker = lpp_link_binary();
    if !linker.exists() { return Err("lpp-link binary not found".into()); }
    let start = Instant::now();
    let out = Command::new(&linker)
        .arg(obj).arg(runtime_obj).arg("-o").arg(output)
        .stdout(Stdio::piped()).stderr(Stdio::piped())
        .spawn().map_err(|e| format!("spawn: {e}"))?
        .wait_with_output().map_err(|e| format!("wait: {e}"))?;
    Ok(LinkerResult {
...
fn run_linker_host(
    obj: &Path, runtime_src: &Path, output: &Path,
    ...
    if cc.contains("cl.exe") || cc == "cl" {
        cmd.arg("/nologo").arg("/O2")
           .arg(obj).arg(runtime_src)
           .arg(format!("/Fe:{}", output.display()));
    } else {
        cmd.arg("-O2").arg(obj).arg(runtime_src).arg("-o").arg(output);
        if !cfg!(target_os = "macos") { cmd.arg("-pthread"); }
    }

    let out = cmd.stdout(Stdio::piped()).stderr(Stdio::piped())
        .spawn().map_err(|e| format!("spawn {cc}: {e}"))?
        .wait_with_output().map_err(|e| format!("wait: {e}"))?;
    let label = if has_mold() { "mold via ".to_string() + &cc }
                else { cc };
...
fn run_linker_c_transpile(
    source: &Path, runtime_src: &Path, output: &Path, lpp: &Path,
) -> Result<LinkerResult, String> {
    let _p = plat();
    let cc = detect_c_compiler().ok_or("no C compiler found")?;
    let transpile_start = Instant::now();

    if !Command::new(lpp).arg(source)
        .stdout(Stdio::null()).stderr(Stdio::null())
        .status().map(|s| s.success()).unwrap_or(false)
    { return Err("transpile failed".into()); }

    let c_path = source.with_extension("c");
    if !c_path.exists() { return Err("C file not generated".into()); }

    let start = Instant::now();
    let mut cmd = Command::new(&cc);
    if cc.contains("cl.exe") || cc == "cl" {
        cmd.arg("/nologo").arg("/O2").arg(&c_path).arg(runtime_src)
           .arg(format!("/Fe:{}", output.display()));
    } else {
        cmd.arg("-O2").arg(&c_path).arg(runtime_src).arg("-o").arg(output);
        if !cfg!(target_os = "macos") { cmd.arg("-pthread"); }
    }
    let out = cmd.stdout(Stdio::piped()).stderr(Stdio::piped())
        .spawn().map_err(|e| format!("C compile: {e}"))?
        .wait_with_output().map_err(|e| format!("wait: {e}"))?;
    let _ = fs::remove_file(&c_path);
    Ok(LinkerResult {
        linker: "C transpiler".into(),
        link_time_ms: (transpile_start.elapsed().as_secs_f64() + start.elapsed().as_secs_f64()) * 1000.0,
        binary_size_bytes: file_size_bytes(output),
        peak_rss_kb: read_peak_rss_kb(),
        exit_code: out.status.code(),
        stdout: String::from_utf8_lossy(&out.stdout).to_string(),
...
fn run_full_bench(linkers: &[LinkerMode], show_disk: bool, show_mem: bool) -> BenchReport {
    ...
    for case in &workloads {
        ...
        let ok = Command::new(&lpp)
            ...
            .arg(&tmp_src).stdout(Stdio::null()).stderr(Stdio::null())
            .status().map(|s| s.success()).unwrap_or(false);
        let compile_ms = compile_start.elapsed().as_secs_f64() * 1000.0;

        if !ok { eprintln!("  SKIP: compile failed"); continue; }

        let obj_path = if tmp_obj.exists() { tmp_obj.clone() } else { tmp_src.with_extension(p.obj_suffix) };
        let mut linker_results = Vec::new();

        for mode in linkers {
            let exe = tmp.join(format!("{}_{}{}", case.name, mode.short_label(), p.exe_suffix));
            let r = match mode {
                LinkerMode::Direct => {
                ...
                LinkerMode::CTranspiler => run_linker_c_transpile(&tmp_src, &runtime_src, &exe, &lpp),
            }.unwrap_or_else(|e| LinkerResult {
                linker: mode.short_label().into(), link_time_ms: 0.0,
                binary_size_bytes: 0, peak_rss_kb: None, exit_code: Some(1),
                stdout: String::new(), stderr: e, success: false,
            });
            ...
            let (run_stdout, run_exit) = if r.success && exe.exists() {
                ...
                } else { (String::new(), Some(1)) }
            } else { (String::new(), None) };

            let verdict = if run_stdout == case.expected { "PASS" } else { "FAIL" };
            eprintln!("  {} | {:>8} | {}ms | {}",
                mode.short_label(), verdict, r.link_time_ms as u64,
                if r.success { format!("{}KB", r.binary_size_bytes / 1024) } else { "FAIL".into() });
        ...
        let overall = if linker_results.iter().all(|lr| lr.success) { "PASS" }
            else if linker_results.iter().any(|lr| lr.success) { "PARTIAL" } else { "FAIL" };
        results.push(BenchmarkResult {
            name: case.name.clone(),
            source_path: case.source.to_string_lossy().to_string(),
            expected: case.expected.clone(),
            compile_time_ms: compile_ms, linkers: linker_results,
    ...
    // Summary
    let mut dir_p = 0usize; let mut dir_ms = 0.0; let mut dir_kb = 0.0; let mut dir_rss = 0.0; let mut dir_n = 0u64;
    let mut host_p = 0usize; let mut host_ms = 0.0; let mut host_kb = 0.0; let mut host_rss = 0.0; let mut host_n = 0u64;
    let mut c_p = 0usize; let mut c_ms = 0.0; let mut c_kb = 0.0; let mut c_rss = 0.0; let mut c_n = 0u64;
    ...
    for w in &results {
        for lr in &w.linkers {
            match lr.linker.as_str() {
                ...
                _ => { if lr.success { host_p += 1; } host_ms += lr.link_time_ms; host_kb += lr.binary_size_bytes as f64; if let Some(r) = lr.peak_rss_kb { host_rss += r as f64; } host_n += 1; }
            }
        }
    }
    if dir_n > 0 { dir_ms /= dir_n as f64; dir_kb /= dir_n as f64; dir_rss /= dir_n as f64; }
    if host_n > 0 { host_ms /= host_n as f64; host_kb /= host_n as f64; host_rss /= host_n as f64; }
    if c_n > 0 { c_ms /= c_n as f64; c_kb /= c_n as f64; c_rss /= c_n as f64; }

    let summary = LinkerSummary {
        direct_passed: dir_p, host_passed: host_p, c_transpiler_passed: c_p,
        direct_mean_link_ms: dir_ms, host_mean_link_ms: host_ms, c_mean_link_ms: c_ms,
        direct_mean_binary_kb: dir_kb / 1024.0, host_mean_binary_kb: host_kb / 1024.0, c_mean_binary_kb: c_kb / 1024.0,
        direct_mean_rss_mb: dir_rss / 1024.0, host_mean_rss_mb: host_rss / 1024.0, c_mean_rss_mb: c_rss / 1024.0,
    };
    ...
    if show_mem {
        if let Some(rss) = read_peak_rss_kb() { eprintln!("  Peak RSS:         {} MB", rss as f64 / 1024.0); }
    }
...
fn run_self_tests() -> bool {
    ...
    for (name, src_rel, expected) in &cases {
        ...
        if !Command::new(&lpp).env("LPP_AOT", "1").env("LPP_AOT_ONLY", "1")
            .arg(&tmp_src).stdout(Stdio::null()).stderr(Stdio::null())
            .status().map(|s| s.success()).unwrap_or(false)
        { eprintln!("  FAIL {name}: compile"); failed += 1; continue; }
        ...
        let linked = if let Some(ref rt) = runtime_obj {
            linker.exists() && Command::new(&linker).arg(&obj).arg(rt).arg("-o").arg(&exe)
                .stdout(Stdio::null()).stderr(Stdio::null())
                .status().map(|s| s.success()).unwrap_or(false)
        } else { false };
        ...
        if !linked {
            if let Some(ref cc) = cc {
                ...
                let mut cmd = Command::new(cc);
                if cc.contains("cl.exe") || cc == "cl" {
                    cmd.args(["/nologo", "/O2", &obj.to_string_lossy(), &runtime.to_string_lossy(),
                              &format!("/Fe:{}", exe.display())]);
                } else {
                    cmd.args(["-O2", &obj.to_string_lossy(), &runtime.to_string_lossy(),
        ...
        if exe.exists() {
            if let Ok(out) = Command::new(&exe).output() {
                ...
                if stdout == *expected { println!("  PASS {name}"); passed += 1; }
                else { eprintln!("  FAIL {name}: expected '{expected}', got '{stdout}'"); failed += 1; }
            } else { eprintln!("  FAIL {name}: exec error"); failed += 1; }
        } else { eprintln!("  FAIL {name}: link failed"); failed += 1; }
...
fn main() {
    ...
    let linkers = args.iter().position(|a| a == "--linkers")
        ...
        .map(|s| {
            ...
            if m.is_empty() { LinkerMode::from_str("direct,host,c") } else { m }
    ...
    if show_json {
        let json = if pretty { serde_json::to_string_pretty(&report) }
                   else { serde_json::to_string(&report) };
        if let Ok(j) = json { println!("{j}"); }
    }
    ...
    eprintln!(" host      | {:>6} | {:>8.2} | {:>10.1} | {:>7.2}",
        report.summary.host_passed, report.summary.host_mean_link_ms,
        report.summary.host_mean_binary_kb, report.summary.host_mean_rss_mb);
    ...
    std::process::exit(if total_ok == report.total_workloads { 0 } else { 1 });
}
