...
impl<'a> Codegen<'a> {
    ...
    pub fn generate(&mut self, program: &Program) -> String {
        ...
        self.out.push_str("struct JsonNode {\n");
        self.out.push_str("    char *key;\n");
        self.out.push_str("    int type;\n");
        self.out.push_str("    union {\n");
        self.out.push_str("        int64_t int_val;\n");
        self.out.push_str("        char *str_val;\n");
        self.out.push_str("        struct JsonNode *obj_val;\n");
        self.out.push_str("    } value;\n");
        self.out.push_str("    struct JsonNode *next;\n");
        self.out.push_str("};\n\n");

        self.out.push_str("static void skip_json_ws(const char **p) {\n");
        self.out.push_str("    while (**p == ' ' || **p == '\\t' || **p == '\\r' || **p == '\\n') {\n");
        self.out.push_str("        (*p)++;\n");
        self.out.push_str("    }\n");
        self.out.push_str("}\n\n");

        self.out.push_str("static char *parse_json_string(const char **p) {\n");
        self.out.push_str("    skip_json_ws(p);\n");
        self.out.push_str("    if (**p != '\"') return NULL;\n");
        self.out.push_str("    (*p)++;\n");
        self.out.push_str("    const char *start = *p;\n");
        self.out.push_str("    while (**p && **p != '\"') {\n");
        self.out.push_str("        (*p)++;\n");
        self.out.push_str("    }\n");
        self.out.push_str("    size_t len = *p - start;\n");
        self.out.push_str("    char *res = malloc(len + 1);\n");
        self.out.push_str("    memcpy(res, start, len);\n");
        self.out.push_str("    res[len] = '\\0';\n");
        self.out.push_str("    if (**p == '\"') (*p)++;\n");
        self.out.push_str("    return res;\n");
        self.out.push_str("}\n\n");

        self.out.push_str("static struct JsonNode *parse_json_object(const char **p);\n\n");

        self.out.push_str("static struct JsonNode *parse_json_value(const char **p) {\n");
        self.out.push_str("    skip_json_ws(p);\n");
        self.out.push_str("    if (**p == '{') {\n");
        self.out.push_str("        return parse_json_object(p);\n");
        self.out.push_str("    } else if (**p == '\"') {\n");
        self.out.push_str("        char *s = parse_json_string(p);\n");
        self.out.push_str("        struct JsonNode *n = calloc(1, sizeof(struct JsonNode));\n");
        self.out.push_str("        n->type = 1;\n");
        self.out.push_str("        n->value.str_val = s;\n");
        self.out.push_str("        return n;\n");
        self.out.push_str("    } else if ((**p >= '0' && **p <= '9') || **p == '-') {\n");
        self.out.push_str("        char *end;\n");
        self.out.push_str("        long long val = strtoll(*p, &end, 10);\n");
        self.out.push_str("        *p = end;\n");
        self.out.push_str("        struct JsonNode *n = calloc(1, sizeof(struct JsonNode));\n");
        self.out.push_str("        n->type = 0;\n");
        self.out.push_str("        n->value.int_val = (int64_t)val;\n");
        self.out.push_str("        return n;\n");
        self.out.push_str("    }\n");
        ...
        self.out.push_str("    if (**p != '{') return NULL;\n");
        self.out.push_str("    (*p)++;\n");
        self.out.push_str("    struct JsonNode *head = NULL;\n");
        self.out.push_str("    struct JsonNode *tail = NULL;\n");
        self.out.push_str("    while (**p && **p != '}') {\n");
        self.out.push_str("        skip_json_ws(p);\n");
        self.out.push_str("        if (**p == '}') break;\n");
        self.out.push_str("        char *key = parse_json_string(p);\n");
        self.out.push_str("        skip_json_ws(p);\n");
        self.out.push_str("        if (**p != ':') {\n");
        self.out.push_str("            free(key);\n");
        self.out.push_str("            break;\n");
        self.out.push_str("        }\n");
        self.out.push_str("        (*p)++;\n");
        self.out.push_str("        struct JsonNode *val = parse_json_value(p);\n");
        self.out.push_str("        if (val) {\n");
        self.out.push_str("            val->key = key;\n");
        self.out.push_str("            if (!head) {\n");
        self.out.push_str("                head = val;\n");
        self.out.push_str("                tail = val;\n");
        self.out.push_str("            } else {\n");
        self.out.push_str("                tail->next = val;\n");
        self.out.push_str("                tail = val;\n");
        self.out.push_str("            }\n");
        self.out.push_str("        } else {\n");
        self.out.push_str("            free(key);\n");
        ...
        self.out.push_str("    if (**p == '}') (*p)++;\n");
        self.out.push_str("    struct JsonNode *n = calloc(1, sizeof(struct JsonNode));\n");
        self.out.push_str("    n->type = 2;\n");
        self.out.push_str("    n->value.obj_val = head;\n");
        self.out.push_str("    return n;\n");
        self.out.push_str("}\n\n");

        self.out.push_str("static int64_t json_parse(const char *str) {\n");
        self.out.push_str("    if (!str) return 0;\n");
        self.out.push_str("    const char *p = str;\n");
        self.out.push_str("    return (int64_t)parse_json_value(&p);\n");
        self.out.push_str("}\n\n");

        self.out.push_str("static int64_t json_get_int(int64_t json, const char *key) {\n");
        self.out.push_str("    struct JsonNode *node = (struct JsonNode *)json;\n");
        self.out.push_str("    if (!node) return 0;\n");
        self.out.push_str("    if (node->type == 2) {\n");
        self.out.push_str("        struct JsonNode *curr = node->value.obj_val;\n");
        self.out.push_str("        while (curr) {\n");
        self.out.push_str("            if (curr->key && strcmp(curr->key, key) == 0) {\n");
        self.out.push_str("                if (curr->type == 0) return curr->value.int_val;\n");
        self.out.push_str("                return 0;\n");
        self.out.push_str("            }\n");
        self.out.push_str("            curr = curr->next;\n");
        self.out.push_str("        }\n");
        self.out.push_str("    }\n");
        self.out.push_str("    return 0;\n");
        self.out.push_str("}\n\n");

        self.out.push_str("static char* json_get_str(int64_t json, const char *key) {\n");
        self.out.push_str("    struct JsonNode *node = (struct JsonNode *)json;\n");
        self.out.push_str("    if (!node) return \"\";\n");
        self.out.push_str("    if (node->type == 2) {\n");
        self.out.push_str("        struct JsonNode *curr = node->value.obj_val;\n");
        self.out.push_str("        while (curr) {\n");
        self.out.push_str("            if (curr->key && strcmp(curr->key, key) == 0) {\n");
        self.out.push_str("                if (curr->type == 1) return curr->value.str_val ? curr->value.str_val : \"\";\n");
        ...
        self.out.push_str("static int64_t json_get_obj(int64_t json, const char *key) {\n");
        self.out.push_str("    struct JsonNode *node = (struct JsonNode *)json;\n");
        self.out.push_str("    if (!node) return 0;\n");
        self.out.push_str("    if (node->type == 2) {\n");
        self.out.push_str("        struct JsonNode *curr = node->value.obj_val;\n");
        self.out.push_str("        while (curr) {\n");
        self.out.push_str("            if (curr->key && strcmp(curr->key, key) == 0) {\n");
        self.out.push_str("                if (curr->type == 2) return (int64_t)curr;\n");
        self.out.push_str("                return 0;\n");
        self.out.push_str("            }\n");
        self.out.push_str("            curr = curr->next;\n");
        self.out.push_str("        }\n");
        ...
        self.out.push_str("static void json_free_node(struct JsonNode *node) {\n");
        self.out.push_str("    if (!node) return;\n");
        self.out.push_str("    if (node->key) free(node->key);\n");
        self.out.push_str("    if (node->type == 1) {\n");
        self.out.push_str("        if (node->value.str_val) free(node->value.str_val);\n");
        self.out.push_str("    } else if (node->type == 2) {\n");
        self.out.push_str("        struct JsonNode *curr = node->value.obj_val;\n");
        self.out.push_str("        while (curr) {\n");
        self.out.push_str("            struct JsonNode *next = curr->next;\n");
        self.out.push_str("            json_free_node(curr);\n");
        self.out.push_str("            curr = next;\n");
        self.out.push_str("        }\n");
        self.out.push_str("    }\n");
        self.out.push_str("    free(node);\n");
        self.out.push_str("}\n\n");

        self.out.push_str("static void json_free(int64_t json) {\n");
        self.out.push_str("    json_free_node((struct JsonNode *)json);\n");
        self.out.push_str("}\n\n");
