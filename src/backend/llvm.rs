...
fn builtin_signature(symbol: &str) -> Option<(&'static str, &'static str)> {
    Some(match symbol {
        ...
        "lpp_gui_draw_rounded_rect" | "gui_draw_rounded_rect" => {
        ...
        "lpp_buf_get64le" | "buf_get64le" | "lpp_buf_get64be" | "buf_get64be" => {
            ("i64", "ptr, i64")
        }
        "lpp_buf_set64le" | "buf_set64le" | "lpp_buf_set64be" | "buf_set64be" => {
            ("void", "ptr, i64, i64")
        }
        ...
        "lpp_net_send" | "net_send" | "lpp_net_send_all" | "net_send_all" => {
            ("i64", "i64, ptr, i64")
        }
...
impl<'a> FunctionEmitter<'a> {
    ...
    fn local_ptr(id: LocalId) -> String {
        format!("%l{}", id.0)
    }
    ...
    fn field_ptr(
        ...
        field: &str,
        out: &mut String,
    ) -> Result<(String, TypeRef), String> {
        let definition = self
            .type_table
            .definitions
            .get(sid.0)
            .ok_or_else(|| "unknown LLVM struct".to_string())?;
        let index = definition
            .fields
            .iter()
            .position(|(name, _)| name == field)
            .ok_or_else(|| format!("unknown field '{}.{}'", definition.name, field))?;
    ...
    fn register_builtin(&mut self, symbol: &str) -> Result<(&'static str, &'static str), String> {
        let (ret, params) = builtin_signature(symbol)
            .ok_or_else(|| format!("LLVM backend does not support builtin '{}'", symbol))?;
        self.declarations
            .insert(symbol.to_string(), (ret.to_string(), params.to_string()));
        Ok((ret, params))
    }
    ...
    fn rvalue(
        ...
        match rvalue {
            ...
            Rvalue::TupleField(base, index) => {
                ...
                let field = layout
                    .get(*index)
                    .ok_or_else(|| "LLVM tuple index out of range".to_string())?;
            ...
            Rvalue::SliceToStr(view) => {
                ...
                let value = self.temp();
                out.push_str(&format!(
                    "  {} = call ptr @lpp_str_slice_to_str(ptr {})\n",
                    value, view
                ));
                Ok(Some((value, "ptr")))
            }
            Rvalue::MakeTask(id, argument_types, arguments, result_type) => {
                ...
                for ((operand, field), ty) in arguments
                    .iter()
                    .zip(layout.iter())
            ...
            Rvalue::CallDirect(id, args) => {
                let target = self
                    .signatures
                    .get(id)
                    .ok_or_else(|| format!("unknown LLVM callee {:?}", id))?
                    .clone();
            ...
            Rvalue::BuiltinCall(symbol, args) => {
                if matches!(
                    ...
                    let operand =
                        |this: &mut Self, op: &Operand, out: &mut String| this.operand(op, out);
                    let vector =
                        |this: &mut Self, op: &Operand, out: &mut String| operand(this, op, out);
                    let result = match symbol.as_str() {
                        "lpp_vec_i64x2" => {
                            if args.len() != 2 {
                                return Err("vec_i64x2 requires two lanes".to_string());
                            }
                            let (first, _) = operand(self, &args[0], out)?;
                            let mut value = self.temp();
                            out.push_str(&format!(
                                "  {} = insertelement {} poison, i64 {}, i32 0\n",
                        ...
                        | "lpp_vec_i64x2_shr_var" => {
                            ...
                            let right = if symbol == "lpp_vec_i64x2_shr" {
                                ...
                                for index in 1..2 {
                                    ...
                                    out.push_str(&format!(
                                        "  {} = insertelement {} {}, i64 {}, i32 {}\n",
                                        next, vector_ty, splat, right_raw, index
                                    ));
                                    splat = next;
                                }
                                splat
                            ...
                            let next = self.temp();
                            let instruction = match symbol.as_str() {
                                "lpp_vec_i64x2_add" => "add",
                                "lpp_vec_i64x2_sub" => "sub",
                                "lpp_vec_i64x2_mul" => "mul",
                                "lpp_vec_i64x2_xor" => "xor",
                                _ => "ashr",
                            };
                            out.push_str(&format!(
                                "  {} = {} {} {}, {}\n",
                        ...
                        "lpp_vec_i64x2_extract" => {
                            ...
                            let lane = match args.get(1) {
                                Some(Operand::Int(index)) if (0..2).contains(index) => *index,
                                _ => {
                                    return Err(
                                        "vector extract lane must be constant 0..3".to_string()
                                    );
                                }
                            };
                            let next = self.temp();
                ...
                if symbol == "lpp_vec_i64_checksum" {
                    let arg = args
                        .first()
                        .ok_or_else(|| "vector checksum needs a length".to_string())?;
                    let (n, _) = self.operand(arg, out)?;
                    let value = self.temp();
                    out.push_str(&format!(
                        "  {} = call i64 @__lpp_vec_i64_checksum(i64 {})\n",
                        value, n
                    ));
            ...
            Rvalue::FieldAccess(base, field) => {
                let sid = match base {
                    Operand::Local(id) | Operand::Borrowed(id) => {
                        match self.function.locals[id.0].ty {
            ...
            Rvalue::AllocateArcStruct(TypeRef::Custom(sid)) => {
                ...
                self.declarations.insert(
                    "lpp_arc_alloc_with_destructor".to_string(),
                    ("ptr".to_string(), "i64, ptr".to_string()),
                );
                out.push_str(&format!(
                    "  {} = call ptr @lpp_arc_alloc_with_destructor(i64 {}, ptr {})\n",
            ...
            Rvalue::AllocateArenaStruct(TypeRef::Custom(sid), arena) => {
                ...
                let drop_name = format!("@__lpp_drop_{}", sid.0);
                self.declarations.insert(
                    "lpp_arena_alloc".to_string(),
                    ("ptr".to_string(), "i64, i64, ptr".to_string()),
                );
                out.push_str(&format!(
                    "  {} = call ptr @lpp_arena_alloc(i64 {}, i64 {}, ptr {})\n",
            ...
            Rvalue::AllocateStackStruct(TypeRef::Custom(sid)) => {
                ...
                self.declarations.insert(
                    "llvm.memset.p0.i64".to_string(),
                    ("void".to_string(), "ptr, i8, i64, i1".to_string()),
                );
                Ok(Some((value, "ptr")))
            }
            ...
            Rvalue::MakeClosure(id, args) | Rvalue::MakeStackClosure(id, args) => {
                ...
                if stack {
                    out.push_str(&format!("  {} = alloca [16 x i8], align 8\n", capsule));
                } else {
                    self.declarations.insert(
                        "lpp_arc_alloc_with_destructor".to_string(),
                        ("ptr".to_string(), "i64, ptr".to_string()),
                ...
                out.push_str(&format!(
                    ...
                    self.func_names
                        .get(id)
                        .ok_or_else(|| "unknown closure function".to_string())?
                ));
                out.push_str(&format!("  store ptr {}, ptr {}, align 8\n", code, capsule));
                let env = args
                    .first()
                    .ok_or_else(|| "closure missing environment".to_string())?;
            ...
            Rvalue::SpawnThread(closure) => {
                ...
                out.push_str(&format!(
                    "  call void @lpp_thread_spawn(ptr {}, ptr {})\n",
                    code, env
                ));
            ...
            Rvalue::FuncRef(id) => {
                ...
                out.push_str(&format!(
                    ...
                    self.func_names
                        .get(id)
                        .ok_or_else(|| "unknown function reference".to_string())?
                ));
                Ok(Some((value, "i64")))
            }
            Rvalue::AllocateStruct(_) => {
                Err("raw struct allocation reached LLVM backend".to_string())
            }
            _ => Err(format!(
                "LLVM backend does not support MIR rvalue {}",
                rvalue
            )),
        }
    }
    ...
    fn emit(&mut self) -> Result<String, String> {
        ...
        body.push_str(&format!(
            "define internal {} {}(",
            return_type, self.func_names[&self.function.id]
        ));
        let args: Vec<String> = self
            ...
            .iter()
            .enumerate()
            .map(|(i, p)| format!("{} %arg{}", llvm_type(&self.function.locals[p.0].ty), i))
            .collect();
        ...
        for local in &self.function.locals {
            if local.ty != TypeRef::Void {
                ...
                let zero_val = if local.ty == TypeRef::Float {
                    "0.0"
                } else if ty_str == "ptr" {
                    "null"
                } else {
        ...
        for (i, param) in self.function.params.iter().enumerate() {
            if self.function.locals[param.0].ty != TypeRef::Void {
                body.push_str(&format!(
                    "  store {} %arg{}, ptr {}, align 8\n",
                    llvm_type(&self.function.locals[param.0].ty),
                    i,
                    Self::local_ptr(*param)
                ));
            }
        }
        let first = self
            ...
            .blocks
            .first()
            .ok_or_else(|| "LLVM function has no blocks".to_string())?;
        body.push_str(&format!("  br label %bb{}\n", first.id.0));
        for block in &self.function.blocks {
            body.push_str(&format!("bb{}:\n", block.id.0));
            for instruction in &block.instrs {
                match instruction {
                    MirInstr::Assign(dest, rv) => {
                        if let Some((value, ty)) =
                            self.rvalue(rv, &self.function.locals[dest.0].ty, &mut body)?
                        {
                            if self.function.locals[dest.0].ty != TypeRef::Void {
                                self.store(*dest, &value, ty, &mut body);
                            }
                        }
                    }
                    MirInstr::AssignField { base, field, value } => {
                        let (base_value, _) = self.operand(&Operand::Local(*base), &mut body)?;
                        let sid = match &self.function.locals[base.0].ty {
                            TypeRef::Custom(id) => *id,
                            _ => {
                                return Err(
                                    "LLVM field store base is not a custom struct".to_string()
                                );
                            }
                        };
                        let (address, field_ty) =
                            self.field_ptr(&base_value, sid, field, &mut body)?;
                        let (raw_value, actual_ty) = self.operand(value, &mut body)?;
                        let value =
                            self.coerce(&raw_value, actual_ty, llvm_type(&field_ty), &mut body);
                        body.push_str(&format!(
                            "  store {} {}, ptr {}, align 8\n",
                            llvm_type(&field_ty),
                            value,
                    ...
                    MirInstr::Release(local) => {
                        ...
                        if self.function.locals[local.0].ownership.is_copy() {
                            match self.function.locals[local.0].ty {
                                TypeRef::Custom(id) => body.push_str(&format!(
                                    "  call void @__lpp_drop_{}(ptr {})\n",
                                    id.0, value
                                )),
                                TypeRef::Function => {
                                    self.register_builtin("lpp_closure_destroy")?;
                                    body.push_str(&format!(
                                        "  call void @lpp_closure_destroy(ptr {})\n",
                                        value
                                    ));
                                }
                                _ => {}
            ...
            match &block.terminator {
                ...
                Terminator::If {
                    ...
                    let (value, ty) = self.operand(cond, &mut body)?;
                    let test = self.temp();
                    if ty == "ptr" {
                        body.push_str(&format!("  {} = icmp ne ptr {}, null\n", test, value));
                    } else {
                        body.push_str(&format!("  {} = icmp ne {} {}, 0\n", test, ty, value));
                    }
                    body.push_str(&format!(
                ...
                Terminator::IfCmp {
                    ...
                    let (left, ty) = self.operand(left, &mut body)?;
                    let (right, rty) = self.operand(right, &mut body)?;
                    let right = self.coerce(&right, rty, ty, &mut body);
                    let test = self.compare(op, &left, &right, ty, &mut body);
                    body.push_str(&format!(
                        "  br i1 {}, label %bb{}, label %bb{}\n",
                        test, then_block.0, else_block.0
                    ));
                ...
                Terminator::Return(Some(op)) | Terminator::ReturnOwned(op) => {
                    let (value, ty) = self.operand(op, &mut body)?;
                    body.push_str(&format!("  ret {} {}\n", ty, value));
                }
                Terminator::Return(None) => {
                    if return_type == "void" {
                        body.push_str("  ret void\n");
                    } else if return_type == "ptr" {
                        body.push_str("  ret ptr null\n");
                    } else if return_type == "double" {
                        body.push_str("  ret double 0.0\n");
                    } else {
                        body.push_str(&format!("  ret {} 0\n", return_type));
                    }
                }
...
fn emit_drop_functions(
    ...
    let mut out = String::new();
    declarations.insert(
        "lpp_arc_release".to_string(),
        ("void".to_string(), "ptr".to_string()),
    );
    declarations.insert(
        "lpp_arena_release_node".to_string(),
        ("void".to_string(), "ptr".to_string()),
    );
    for (index, definition) in type_table.definitions.iter().enumerate() {
        out.push_str(&format!(
            "define internal void @__lpp_drop_{}(ptr %payload) {{\nentry:\n",
            index
        ));
        ...
        for ((field, ty), field_layout) in definition.fields.iter().zip(layout.iter()) {
            if weak_fields.contains(&(StructTypeId(index), field.clone())) || !ty.is_managed() {
                continue;
            }
            let ptr = format!("%field{}_{}", index, field_layout.offset);
            out.push_str(&format!(
                "  {} = getelementptr i8, ptr %payload, i64 {}\n",
                ptr, field_layout.offset
            ));
            let value = format!("%value{}_{}", index, field_layout.offset);
            out.push_str(&format!("  {} = load ptr, ptr {}, align 8\n", value, ptr));
            let release = match ty {
                TypeRef::Custom(id)
                    if type_table
                        .definitions
                        .get(id.0)
                        .map(|d| d.is_self_referential)
                        .unwrap_or(false) =>
                {
                    "lpp_arena_release_node"
                }
...
fn emit_task_thunks(program: &MirProgram, func_names: &HashMap<FuncId, String>) -> String {
...
pub fn compile(
    ...
    for function in program.functions.values() {
        if !is_supported_type(&function.return_type)
            || function
                .locals
                .iter()
                .any(|local| !is_supported_type(&local.ty))
        {
            return Err(format!(
                "LLVM backend cannot lower unsupported type in '{}'",
                function.name
            ));
        }
    }
    ...
    let mut ordered: Vec<&MirFunction> = program.functions.values().collect();
    ordered.sort_by_key(|f| f.id.0);
    for function in &ordered {
        let name = format!("@lpp_fn_{}", function.id.0);
        let params: Vec<&'static str> = function
            .params
            .iter()
    ...
    {
        declarations.insert(
            "lpp_tuple_alloc".to_string(),
            ("ptr".to_string(), "i64, i64, i64".to_string()),
        );
        declarations.insert(
            "lpp_task_new".to_string(),
            ("ptr".to_string(), "ptr, ptr, i64".to_string()),
        );
        declarations.insert(
            "lpp_task_await".to_string(),
            ("i64".to_string(), "ptr".to_string()),
        );
        declarations.insert(
            "lpp_arc_release".to_string(),
            ("void".to_string(), "ptr".to_string()),
    ...
    for (i, value) in strings.iter().enumerate() {
        let blob = literal_blob(value);
        ir.push_str(&format!(
            "@.lpp_str{} = private unnamed_addr constant [{} x i8] c\"{}\", align 16\n",
            i,
            blob.len(),
            escape_bytes(&blob)
        ));
    ...
    ir.push_str(&drops);
    ir.push_str(&emit_task_thunks(program, &func_names));
    ir.push_str(emit_vector_checksum());
